...
N="saumil-codes";WK=62
Z,G,P,Y=12,3,34,262;S=Z+G;M=18
L=["#2B313A","#0E4429","#006D32","#26A641","#39D353"]
Q="""query($u:String!){allQuestionsCount{difficulty count}
matchedUser(username:$u){submitStats{acSubmissionNum{difficulty count submissions}
totalSubmissionNum{difficulty count submissions}}
userCalendar{streak totalActiveDays submissionCalendar}}}"""
def fetch():
 b=json.dumps({"query":Q,"variables":{"u":N}}).encode()
 r=U.Request("https://leetcode.com/graphql",data=b,headers={
  "Content-Type":"application/json","Referer":"https://leetcode.com/u/"+N+"/",
  "User-Agent":"Mozilla/5.0 profile-card"})
...
for a in range(3):
 try:
  ...
  print("attempt",a+1,"failed:",e,file=sys.stderr);time.sleep(8)
if not d or not d.get("data",{}).get("matchedUser"):
 print("no data from leetcode",file=sys.stderr);sys.exit(1)
D0=d["data"];mu=D0["matchedUser"]
T={x["difficulty"]:x["count"] for x in D0["allQuestionsCount"]}
ss=mu["submitStats"]
C={x["difficulty"]:x["count"] for x in ss["acSubmissionNum"]}
tot=C.get("All",0)
sb=next(x["submissions"] for x in ss["totalSubmissionNum"] if x["difficulty"]=="All")
ac=next(x["submissions"] for x in ss["acSubmissionNum"] if x["difficulty"]=="All")
rt=round(100*ac/sb,1) if sb else 0
uc=mu["userCalendar"];st=uc.get("streak",0);ad=uc.get("totalActiveDays",0)
D={datetime.fromtimestamp(int(k),timezone.utc).date():x for k,x in json.loads(uc["submissionCalendar"]).items()}
...
cl=mo="";x=P;y0,m0=bg.year,bg.month
while (y0,m0)<=(td.year,td.month):
 ...
 while dd<=e:
  w=(dd.weekday()+1)%7
  if dd>f and w==0:col+=1
  cl+=f'<rect x="{x+col*S}" y="{Y+w*S}" width="{Z}" height="{Z}" rx="2" fill="{L[v(D.get(dd,0))]}"/>'
  dd+=timedelta(days=1)
 bw=(col+1)*S-G
...
for i,k in enumerate(K):
 sg=CC*C.get(k,0)/tot if tot else 0
...
rw=""
for i,k in enumerate(K):
 yy=104+i*44;fw=max(7,b2*C.get(k,0)/T.get(k,1))
 rw+=(f'<text x="{bx}" y="{yy}" fill="#AEB6C4" font-size="14" font-weight="600">{k}</text>'
 f'<text x="{W-P}" y="{yy}" text-anchor="end" font-size="14"><tspan fill="#FFF" font-weight="700">{C.get(k,0)}</tspan><tspan fill="#59616F"> / {T.get(k,0)}</tspan></text>'
